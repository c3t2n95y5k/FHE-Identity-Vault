...
import { minimalProviderSend } from "./provider.js";
/*
...
*/
export function computeStorageLocation(storageName) {
    const enc = EthersT.AbiCoder.defaultAbiCoder().encode(["uint256"], [BigInt(EthersT.keccak256(EthersT.toUtf8Bytes(storageName))) - 1n]);
    return EthersT.toBeHex(BigInt(EthersT.keccak256(enc)) & 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff00n, 32);
}
/**
 ...
 */
export async function getStorageAt(provider, address, index) {
...
export async function setStorageAt(provider, methodName, address, index, valueBytes32) {
    assertIsAddress(address);
 ...
 */
export async function getAddressesFromStorage(provider, contractAddress, storageLocationBytes32, numAddresses) {
    const addresses = [];
...
export async function getInitializableStorage(provider, contractAddress) {
...
export async function setInitializableStorage(provider, contractAddress, value) {
    //
